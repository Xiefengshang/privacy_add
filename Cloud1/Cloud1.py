...
class SocketClient(object):
    def __init__(self, host, port, key1):
        self.HOST = host
        self.PORT = port
        self.KEY = key1
        self._BUFSIZ = 1024
    ...
    def sent_data_pending_key(self):
        """
        发送密钥
        """
        self._tcpCliSock.sendall(self._en_data(self.KEY))
        recv = self._recv_data()
        print(recv)
        print("Client端已退出！")
...
def send_m(host, port, key1):
    with open(key1, "r") as A:
       pub_jwk = A.read()
    A.close()
    cli_obj = SocketClient(host, port, pub_jwk)
    cli_obj.sent_data_pending_key()
    #删除本地保存的privkey
    os.remove(key1)
if __name__ == '__main__':
    # Cloud2接受key的IP和端口
    send_m('192.168.75.128', 48989, "./privkey.pub")
